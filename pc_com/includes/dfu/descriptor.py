...
def get_memory_layout(
    ...
    try:
        try:
            mem_layout_raw = usb.util.get_string(device, intf.iInterface)
        except ValueError as e:
            logger.warning("Skipping device: could not read iInterface string (%s)", e)
            return []
        if not mem_layout_raw:
            return []
        mem_layout_str = mem_layout_raw.split("/")
